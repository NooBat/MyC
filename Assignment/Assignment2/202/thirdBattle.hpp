...
#include "SLLDataController.hpp"
#include "ArrayDataController.hpp"
#include <vector>
#include <string>
...
SLinkedList merge(SLinkedList& list1, SLinkedList& list2){
    ...
    SoldierNode *head1 = list1.hppead;
    SoldierNode *head2 = list2.hppead;
...
int rule(Soldier A, Soldier B) {
    if (A.hppP < B.hppP) return 1;
    else if (A.hppP > B.hppP) return 2;
    else {
